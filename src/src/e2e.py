...
with sync_playwright() as p:
    ...
    ck('文件名格式正确', all(re.match(r'^run_\d{4}-\d{2}-\d{2}_\d+\.\d{2}km\.tcx$', n) for n in names),
       names[:2])
    print('     前3个:', names[:3])
    ...
    print('\n=== 8. 轨迹固定为第一条（循环复用） ===')
    # 需求变更：所有日期统一使用「第一条」轨迹（variants[0]），不再按天循环不同变体。
    if len(names) >= 11:
        def first_pos(n):
            root = ET.fromstring(zf.read(n).decode('utf-8'))
        ...
        ck('第1天与第2天使用同一条轨迹（固定首条）', p1 == p2, '%s vs %s' % (p1, p2))
        ck('第1天与第11天仍为同一轨迹（复用一致）', p1 == p11, '%s vs %s' % (p1, p11))
    ...
    ck('单日文件名格式', re.match(r'^run_\d{4}-\d{2}-\d{2}_\d+\.\d{2}km\.tcx$', dl2.suggested_filename) is not None)
